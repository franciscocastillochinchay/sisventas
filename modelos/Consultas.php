...
public function ventasfechacliente($fecha_inicio,$fecha_fin,$idcliente){
	...
	$sql="SELECT (SELECT p.nombre from persona p where p.tipo_persona='Cliente' and p.idpersona=v.idcliente) cliente,
	(SELECT p.zona from persona p where p.tipo_persona='Cliente' and p.idpersona=v.idcliente) zona,
	v.tipo_comprobante,v.serie_comprobante,v.num_comprobante,v.fecha_hora as fecha,
	v.impuesto,v.total_venta,v.estado,
	(SELECT CONCAT(a.codigo,'-',a.nombre) from articulo a where a.idarticulo=dv.idarticulo) descripcion  ,
	dv.cantidad,dv.precio_venta,dv.descuento,
	(SELECT u.login from usuario u where u.idusuario=v.idusuario) usuario
	from venta v
...
public function kardexfechaItem($fecha_inicio,$fecha_fin){
	//$sql="SELECT DATE(v.fecha_hora) as fecha, u.nombre as usuario, p.nombre as cliente, v.tipo_comprobante,v.serie_comprobante, v.num_comprobante , v.total_venta, v.impuesto, v.estado FROM venta v INNER JOIN persona p ON v.idcliente=p.idpersona INNER JOIN usuario u ON v.idusuario=u.idusuario WHERE DATE(v.fecha_hora)>='$fecha_inicio' AND DATE(v.fecha_hora)<='$fecha_fin' AND v.idcliente='$idcliente'";
	$sql="SELECT tabla.*
	from
	(
	SELECT 'S' s_i,(SELECT p.nombre from persona p where p.tipo_persona='Cliente' and p.idpersona=v.idcliente) cliente,
	v.tipo_comprobante,v.serie_comprobante,v.num_comprobante,v.fecha_hora as fecha,
	v.estado,
	(SELECT CONCAT(a.codigo,'-',a.nombre) from articulo a where a.idarticulo=dv.idarticulo) descripcion  ,
	dv.cantidad
	...
	INNER JOIN detalle_venta dv on v.idventa=dv.idventa
	 ...
	 AND DATE(v.fecha_hora)<='$fecha_fin' 
	UNION
	SELECT 'I' s_i,(SELECT p.nombre from persona p where p.tipo_persona='Proveedor' and p.idpersona=i.idproveedor) proveedor,
		i.tipo_comprobante,i.serie_comprobante,i.num_comprobante,i.fecha_hora as fecha,
	i.estado,
	(SELECT CONCAT(a.codigo,'-',a.nombre) from articulo a where a.idarticulo=di.idarticulo) descripcion  ,
	di.cantidad
	from ingreso i
